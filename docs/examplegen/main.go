...
func run() error {
	...
	driverModules, err := discoverDriverModules(root)
	if err != nil {
		return err
	}
	...
	for _, mod := range driverModules {
		if err := collectExamplesFromDir(funcs, mod.Dir, mod.ImportPath, ""); err != nil {
			return err
		}
	}
	...
	allModules := append([]moduleInfo{{
		Dir:         root,
		ImportPath:  modPath,
		PackageName: filepath.Base(modPath),
	}}, driverModules...)
	...
	for _, fd := range funcs {
		...
		if err := writeMain(examplesDir, fd, fd.ImportPath, allModules); err != nil {
			return err
		}
	}
...
func discoverDriverModules(root string) ([]moduleInfo, error) {
	...
	for _, entry := range entries {
		...
		dir := filepath.Join(driverRoot, entry.Name())
	...
	sort.Slice(modules, func(i, j int) bool { return modules[i].PackageName < modules[j].PackageName })
	return modules, nil
}
	...
	for _, decl := range file.Decls {
		if gen, ok := decl.(*ast.GenDecl); ok && gen.Doc != nil && gen.Tok == token.TYPE {
			for _, spec := range gen.Specs {
				...
				if skipTypeDoc(file.Name.Name, ts.Name.Name) {
					continue
				}
				fd := &FuncDoc{
					Name:        ts.Name.Name,
...
func pruneGeneratedExampleDirs(base, slug string, expected map[string]struct{}) error {
	...
	for dir := range expected {
		targets = append(targets, filepath.Join(base, dir))
	}
	for _, target := range targets {
		if err := pruneIfGeneratedAndUnexpected(target, expected); err != nil {
			return err
		}
	}

	exact := filepath.Join(base, strings.ToLower(slug))
	if err := pruneIfGeneratedAndUnexpected(exact, expected); err != nil {
		return err
	}
	return nil
}
