...
test-integration: ##@tests Run integration tests: make test-integration [all|gcppubsub|kafka|natsjetstream|nats|redis|sns].
	INTEGRATION_DRIVER="$(filter-out all,$(firstword $(ARGS)))" go -C integration test -tags=integration ./...
